...
const (
	...
	envMysqlConnectGKEClusterLoc     = "CLOUDSQL_TEST_GKE_CLUSTER_LOCATION"
	envMysqlConnectGKENamespace      = "CLOUDSQL_TEST_GKE_NAMESPACE"
)
...
func getMysqlConnectGKEParams(t *testing.T) map[string]any {
	...
	if loc := os.Getenv(envMysqlConnectGKEClusterLoc); loc != "" {
		params["cluster_location"] = loc
	}
	if ns := os.Getenv(envMysqlConnectGKENamespace); ns != "" {
		params["namespace"] = ns
	}
...
func TestCloudSQLMySQLConnectGKE(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			...
			params:   mergeMysqlConnectGKEParams(baseParams, map[string]any{"language": "python"}),
		},
	}
...
func mergeMysqlConnectGKEParams(maps ...map[string]any) map[string]any {
