---
title: cloud-sql-postgres-connect-gke
type: docs
weight: 13
description: Connect a Cloud SQL for PostgreSQL instance to a Google Kubernetes Engine cluster — validates Workload Identity and network configuration, recommends a connection method, and emits Kubernetes manifests plus an optional code snippet.
---

## About

The `cloud-sql-postgres-connect-gke` tool helps an agent (or user) wire up a
Cloud SQL for PostgreSQL instance to a Google Kubernetes Engine cluster. It:
...
3. Recommends a primary connection method — typically Cloud SQL Auth Proxy
   sidecar, or Cloud SQL Connector library — plus ranked alternatives.
4. Returns ready-to-paste Kubernetes manifests, environment/DSN/JDBC
   ...
   Java / Go code snippet using the appropriate driver or connector library.
...
| database_name            |  string  |    false     | Database name to connect to. Defaults to `postgres`.                                                                   |
...
tools:
  connect_to_gke:
    ...
    description: Help me connect a Cloud SQL PostgreSQL instance to a GKE cluster.
```
...
- `recommendedMethod` and `alternativeMethods` — one of `auth_proxy` or
  `connector`, with rationale and requirements
...
| ----------- | :------: | :----------: | ------------------------------------------------ |
| type        |  string  |     true     | Must be `cloud-sql-postgres-connect-gke`.        |
