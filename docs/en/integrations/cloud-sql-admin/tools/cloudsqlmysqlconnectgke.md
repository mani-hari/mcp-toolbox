---
title: cloud-sql-mysql-connect-gke
type: docs
weight: 13
description: Connect a Cloud SQL for MySQL instance to a Google Kubernetes Engine cluster — validates Workload Identity and network configuration, recommends a connection method, and emits Kubernetes manifests plus an optional code snippet.
---

## About

The `cloud-sql-mysql-connect-gke` tool helps an agent (or user) wire up a
Cloud SQL for MySQL instance to a Google Kubernetes Engine cluster. It:
...
| database_name            |  string  |    false     | Database name to connect to. Defaults to `mysql`.                                                                      |
...
tools:
  connect_to_gke:
    ...
    description: Help me connect a Cloud SQL MySQL instance to a GKE cluster.
```
...
| ----------- | :------: | :----------: | ------------------------------------------------ |
| type        |  string  |     true     | Must be `cloud-sql-mysql-connect-gke`.           |
