...
// Package cloudsqlmssqlconnectgke provides a tool that connects a Cloud SQL
// for SQL Server instance to a Google Kubernetes Engine cluster: it
// validates Workload Identity and network configuration, recommends an
// Auth Proxy sidecar deployment, and emits ready-to-paste Kubernetes
// manifests and an optional language-specific code snippet. The Cloud SQL
// Connector library supports Postgres and MySQL only, so SQL Server tools
// prefer the Auth Proxy path.
//
...
// for both calls.
package cloudsqlmssqlconnectgke
...
const resourceType string = "cloud-sql-mssql-connect-gke"
...
// Config defines the configuration for the cloud-sql-mssql-connect-gke tool.
type Config struct {
...
func (cfg Config) Initialize(context.Context) (tools.Tool, error) {
	if cfg.Description == "" {
		cfg.Description = "Helps connect a Cloud SQL SQL Server instance to a GKE cluster. " +
...
func buildParams() parameters.Parameters {
	return parameters.Parameters{
		...
		parameters.NewStringParameter(
			...
			parameters.WithStringDefault("master"),
		),
		parameters.NewStringParameter(
			"language",
...
// Tool represents the cloud-sql-mssql-connect-gke tool.
type Tool struct {
...
// Invoke validates connectivity between a Cloud SQL SQL Server instance and a
...
func (t Tool) Invoke(ctx context.Context, s sources.Source, params parameters.ParamValues, accessToken tools.AccessToken) (any, util.ToolboxError) {
	...
	if dbName == "" {
		dbName = cloudsqlconnect.DefaultDatabaseName(cloudsqlconnect.SQLServer)
	}
	...
	if err := cloudsqlconnect.AssertEngine(cloudsqlconnect.SQLServer, instanceName, sqlInfo.DatabaseVersion); err != nil {
		return nil, util.NewAgentError(err.Error(), err)
	}
	...
	port := cloudsqlconnect.GetDatabasePort(cloudsqlconnect.SQLServer)
	...
	connStrings := cloudsqlconnect.BuildConnectionStrings(primary.Method, cloudsqlconnect.SQLServer, sqlInfo, dbName, connName)

	result := &cloudsqlconnect.ConnectResult{
		InstanceConnectionName: connName,
		Project:                project,
		Region:                 region,
		DatabaseType:           cloudsqlconnect.SQLServer,
	...
	if language != "" {
		...
		result.CodeSnippet = cloudsqlconnect.GenerateCodeSnippet(
			lang, primary.Method, cloudsqlconnect.SQLServer,
